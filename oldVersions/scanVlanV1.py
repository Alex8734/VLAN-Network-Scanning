...
def main():
    ...
    # Versuche, die existierende Excel-Datei zu öffnen, oder erstelle eine neue, wenn sie nicht existiert
    try:
        wb = load_workbook('Ergebnisse.xlsx')
        ws_title = f'vlan {VLan}'
        if ws_title in wb.sheetnames:
            # Lösche das existierende Arbeitsblatt
            del wb[ws_title]
        # Erstelle ein neues Arbeitsblatt mit demselben Namen
        ws = wb.create_sheet(title=ws_title)
    except FileNotFoundError:
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = f'vlan {VLan}'
        # Füge die Überschriften hinzu, wenn eine neue Datei erstellt wird
        ws.append(['IP Adresse', 'Port 80', 'Port 443', 'Zertifikat vorhanden', 'System', 'ICMP ECHP', 'HTTP redirect auf HTTPS'])
    ...
    for i in range(1, 256):
        ...
        # Schreibe Ergebnisse in die Excel-Datei
    
        ws.append([ip, port80, port443, certAvailable, system, reachable, redirect])
        print(f'IP: {ip} -- done')
