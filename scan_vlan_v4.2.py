...
async def scan_vlan(VLan, startIdx, endIdx, wb, output_file):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        # Warten auf die Fertigstellung aller Aufgaben und Sammeln der Ergebnisse
        results = [future.result() for future in concurrent.futures.as_completed(futures)]
    

    for result in results:
        ip, port80, port443, certAvailable, system, reachable, redirect,snmp, snmp_devDesc = result
        
        if (port80 != "-" or port443 != "-") and (str.isspace(system) or system == "") and tryAgain:
            try:
                print(f"[Rescanning] IP: {result[0]}...")
                system = get_title_B(f"http://{ip}" if port443 == "-" and port80 != "-" else f"https://{ip}")
                print(f"[Rescanning] IP: {result[0]} --> done")
            except:
                print(f"[Warning] IP: {result[0]} --> failed geting Title")
                

        result = [ip, port80, port443, certAvailable, system, reachable, redirect,snmp, snmp_devDesc]
        ws.append(result)
...
async def main():
    ...
    parser.add_argument('-o', '--file', help='Output file', required=False)
    parser.add_argument('-dt', '--disable-try',action='store_true', help='disable rescan with selenium')
    ...
    tryAgain = False if args.disable_try else True
    if tryAgain:
